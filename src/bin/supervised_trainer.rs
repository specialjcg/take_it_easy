...
fn main() -> Result<()> {
    ...
    log::info!("🎓 Supervised Trainer with Curriculum Learning");
    ...
    let nn_arch = match args.nn_architecture.to_uppercase().as_str() {
        ...
        _ => anyhow::bail!("Invalid architecture: {}", args.nn_architecture),
    };
    ...
    for (phase_idx, data_file) in data_files.iter().enumerate() {
        ...
        log::info!("\n{'='*60}");
        log::info!("📖 Phase {}/{}: {}", phase_num, data_files.len(), data_file);
        log::info!("{'='*60}");
        ...
        log::info!(
            "✅ Loaded {} games with {} total training examples",
            expert_games.len(),
            total_moves
        );
        ...
        manager.save_weights(&checkpoint_path)?;
...
fn load_expert_data(path: &str) -> Result<Vec<ExpertGame>> {
...
fn train_phase(
    ...
    log::info!("✅ Phase {} training complete", phase_num);
...
fn train_epoch(
    ...
    for batch_moves in moves.chunks(batch_size) {
        ...
        let policy_loss = if train_policy {
            let policy_pred = policy_net.forward(&state_tensors);
        ...
        let value_loss = if train_value {
            let value_pred = value_net.forward(&state_tensors);
...
fn encode_state(plateau_before: &[i32], tile: &TileData) -> Vec<f32> {
    ...
    for (cell_idx, &encoded_tile) in plateau_before.iter().enumerate() {
        // Map linear index to 5×5 grid coordinates
        let row = cell_idx / 5;
        let col = cell_idx % 5;
