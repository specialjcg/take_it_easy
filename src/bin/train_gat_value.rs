...
use take_it_easy::neural::model_io::{save_varstore, load_varstore};
use take_it_easy::neural::tensor_conversion::convert_plateau_for_gat_47ch;
use take_it_easy::scoring::scoring::result;

#[derive(Parser, Debug)]
#[command(name = "train_gat_value")]
#[command(about = "Train GAT Value Network to predict final scores")]
struct Args {
    ...
    /// Save model path
    #[arg(long, default_value = "model_weights/gat_value")]
    save_path: String,
    ...
    /// Score normalization: target = (score - mean) / std
    /// mean score for normalization
    #[arg(long, default_value_t = 140.0)]
    score_mean: f64,

    /// std score for normalization
...
fn main() {
    ...
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║       GAT Value Network Training                             ║");
    println!("╚══════════════════════════════════════════════════════════════╝\n");
    ...
    let net = GATValueNet::new(&vs, 47, &hidden, args.heads, args.dropout);
    ...
    for epoch in 0..args.epochs {
        ...
        let saved = if val_loss < best_val_loss {
            ...
            let path = format!("{}_value.safetensors", args.save_path);
    ...
    println!("  Model saved to: {}_value.safetensors", args.save_path);

    // Evaluate on games
    println!("\n🎮 Evaluating value predictions on 100 games...");
    evaluate_value_network(&vs, &net, &args, 100);
}
...
fn evaluate_value_network(_vs: &nn::VarStore, net: &GATValueNet, args: &Args, n_games: usize) {
