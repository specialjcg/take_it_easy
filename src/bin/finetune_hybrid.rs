...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let all_tiles: Vec<Vec<Tile>> = (0..NUM_GAMES)
        .map(|_| sample_tiles(&mut rng, 19))
        .collect();

    // Baseline: Pure MCTS
    println!("Running Pure MCTS baseline...");
    let pure_scores: Vec<i32> = all_tiles.iter()
    ...
    println!("\n✅ Best config: top_k={}, turn_threshold={} -> {:.2} pts (+{:.2} vs pure)",
        best_config.0, best_config.1, best_config.2, best_config.2 - pure_mean);
...
fn play_game_pure(tiles: &[Tile], num_sims: usize) -> i32 {
    ...
    for (turn, tile) in tiles.iter().enumerate() {
        let result = mcts_find_best_position_for_tile_pure(
            &mut plateau, &mut deck, *tile,
            num_sims, turn, 19, None,
        );
...
fn rollout_score(plateau: &take_it_easy::game::plateau::Plateau, deck: &take_it_easy::game::deck::Deck) -> i32 {
    let mut sim_plateau = plateau.clone();
    let mut sim_deck = deck.clone();
    ...
    loop {
        ...
        let empty_positions: Vec<usize> = sim_plateau.tiles.iter()
            .enumerate()
            .filter(|(_, t)| **t == Tile(0, 0, 0))
            .map(|(i, _)| i)
            .collect();

        if empty_positions.is_empty() { break; }
