//! PPO Self-Play RL Training for Graph Transformer
//!
//! Trains the policy via Proximal Policy Optimization in self-play.
//! The value network is trained from scratch; the policy is initialised
//! from the existing supervised GT weights.
//!
//! Usage: cargo run --release --bin rl_selfplay -- --iterations 50
...
struct Args {
    ...
    #[arg(long, default_value = "model_weights/gt_ppo_best.safetensors")]
    save_path: String,
...
struct Trajectory {
    steps: Vec<Step>,
    reward: f64, // normalised final score
}
...
fn collect_trajectories(
    ...
    for _ in 0..n_games {
        ...
        for turn in 0..19 {
            ...
            let feat_batch = feat.unsqueeze(0); // [1, 19, 47]
            ...
            let log_prob = masked_logits.log_softmax(-1, Kind::Float).double_value(&[action]);
        ...
        let reward = (score as f64 - 100.0) / 100.0;
        trajectories.push(Trajectory { steps, reward });
    }
...
fn compute_gae(traj: &Trajectory, gamma: f64, lambda: f64) -> (Vec<f64>, Vec<f64>) {
    ...
    for t in (0..n).rev() {
        ...
        let r = if t == n - 1 { traj.reward } else { 0.0 };
...
fn ppo_update(
    ...
    for _ in 0..args.ppo_epochs {
        ...
        for batch_i in 0..n_batches {
            ...
            let old_log_probs: Vec<f64> = batch_idx.iter().map(|&i| flat[i].old_log_prob).collect();
            ...
            let old_lp_t = Tensor::from_slice(&old_log_probs).to_kind(Kind::Float);
            ...
            // ── Policy forward (no dropout for stable PPO updates) ──
            let logits = policy_net.forward(&feat_t, false);       // [B, 19]
            let masked_logits = logits + &mask_t;
            let log_probs_all = masked_logits.log_softmax(-1, Kind::Float);
            let new_lp = log_probs_all
                .gather(1, &action_t.unsqueeze(1), false)
                .squeeze_dim(1); // [B]

            // Clamp log-prob difference before exp to bound gradients
            // exp(10) ≈ 22k — gradient of exp is bounded, preventing NaN
            let log_ratio = (&new_lp - &old_lp_t).clamp(-10.0, 10.0);
            let ratio = log_ratio.exp();
            let surr1 = &ratio * &adv_t;
            let surr2 = ratio.clamp(1.0 - args.clip_eps, 1.0 + args.clip_eps) * &adv_t;
            let policy_loss = -surr1.min_other(&surr2).mean(Kind::Float);

            // Entropy (over legal positions only)
            // Clamp log_probs to avoid -inf in backward (d(p*log_p)/dp = log_p = -inf → NaN grad)
            let probs = masked_logits.softmax(-1, Kind::Float);
            let safe_log_probs = log_probs_all.clamp_min(-20.0);
            let entropy = -(probs * safe_log_probs).sum_dim_intlist(-1, false, Kind::Float).mean(Kind::Float);

            // ── Value forward ──
            let new_value = value_net.forward(&feat_t, false).squeeze_dim(-1); // [B]
            ...
            // Check for NaN/Inf gradients (clip_grad_norm silently passes NaN)
            let policy_grad_ok = !opt_policy.trainable_variables().iter().any(|t| {
                let g = t.grad();
                g.defined() && (g.isnan().any().int64_value(&[]) != 0
                    || g.isinf().any().int64_value(&[]) != 0)
            });
            if policy_grad_ok {
                opt_policy.clip_grad_norm(0.5);
                opt_policy.step();
            } else {
                nan_batches += 1;
            }
    ...
    if n_updates == 0 {
        return PpoStats { policy_loss: 0.0, value_loss: 0.0, entropy: 0.0, nan_batches };
    }
...
fn main() {
    ...
    println!("==========================================");
    println!("  PPO Self-Play RL Trainer");
    println!("==========================================\n");
    ...
    println!("  PPO epochs:     {}", args.ppo_epochs);
    ...
    println!("  Clip eps:       {}", args.clip_eps);
    ...
    for iter in 0..args.iterations {
        ...
        let avg_score: f64 = trajectories
            .iter()
            .map(|t| t.reward * 100.0 + 100.0)
            .sum::<f64>()
            / trajectories.len() as f64;
        println!(" avg={:.1} pts ({:.1}s)", avg_score, collect_start.elapsed().as_secs_f64());

        // 2. Compute GAE and flatten
        let mut flat_steps: Vec<FlatStep> = Vec::new();
        for traj in &trajectories {
            let (advantages, returns) = compute_gae(traj, args.gamma, args.gae_lambda);
        ...
        // 3. PPO update
        print!("  [2/3] PPO update ({} epochs, bs={})...", args.ppo_epochs, args.batch_size);
        let stats = ppo_update(
            &policy_net,
            &value_net,
            &mut opt_policy,
            &mut opt_value,
    ...
    println!("  Best PPO score:      {:.1} pts", best_score);
    ...
    if best_score > baseline {
        ...
        println!("  GT Direct (best PPO): {:.1} pts (500 games)", final_score);
    }
}
