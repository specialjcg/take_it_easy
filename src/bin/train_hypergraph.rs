...
//!
//! Trains a Hypergraph Transformer that reasons about both positions (19 nodes)
//! and scoring lines (15 hyperedges). Same training pipeline as train_graph_transformer
//! but with the novel hypergraph architecture.
//!
//! Usage:
//!   cargo run --release --bin train_hypergraph -- --device cuda --gen-games 100000 \
...
use take_it_easy::neural::hypergraph_transformer::HypergraphTransformerPolicyNet;
...
struct Args {
    ...
    #[arg(long, default_value = "model_weights/hypergraph")]
    save_path: String,
...
fn main() {
    ...
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║     Hypergraph Transformer Training                          ║");
    println!("╚══════════════════════════════════════════════════════════════╝\n");

    println!("Config:");
    println!("  Device:       {:?}", device);
    println!("  Architecture: Hypergraph Transformer (Nodes + Lines)");
    ...
    // Initialize Hypergraph Transformer
    let vs = nn::VarStore::new(device);
    let policy_net = HypergraphTransformerPolicyNet::new(
        &vs, 47, args.embed_dim, args.num_layers, args.heads, args.dropout,
    );

    let param_count: i64 = vs.variables().values().map(|t| t.numel() as i64).sum();
    println!("   Parameters: {} ({:.1}M)", param_count, param_count as f64 / 1e6);
    ...
    println!("\n Training Hypergraph Transformer...\n");
    for epoch in 0..args.epochs {
        ...
        let (val_loss, val_acc) = evaluate_gpu(
            &policy_net, &features_gpu, &targets_gpu, &masks_gpu,
            &val_indices, args.batch_size, device,
        );
        ...
        if epoch % 5 == 0 || epoch == args.epochs - 1 || improved {
            ...
            println!("Epoch {:3}/{:3} | Train L:{:.4} Acc:{:.1}% | Val L:{:.4} Acc:{:.1}% | {:.1}s{}{}",
                     epoch + 1, args.epochs,
    ...
    println!("══════════════════════════════════════════════════════════════");
    println!("\n  Architecture: Hypergraph Transformer");
    ...
    let (ht_avg, ht_scores) = eval_games(&policy_net, 200, &mut rng, device);

    println!("  Hypergraph Transformer: {:.2} pts", ht_avg);
    println!("  Reference GT Direct:    ~152.3 pts (baseline)");
    println!("\n  vs GT Direct: {:+.2} pts", ht_avg - 152.3);

    let above_100: usize = ht_scores.iter().filter(|&&s| s >= 100).count();
    let above_140: usize = ht_scores.iter().filter(|&&s| s >= 140).count();
    let above_150: usize = ht_scores.iter().filter(|&&s| s >= 150).count();
...
fn generate_selfplay_data(args: &Args, device: Device) -> Vec<Sample> {
    ...
    for game_idx in 0..args.gen_games {
        ...
        for (turn, &tile) in tile_seq.iter().enumerate() {
            ...
            let logits = tch::no_grad(|| gen_net.forward(&feat_device, false))
                .squeeze_dim(0).to_device(Device::Cpu);
        ...
        if final_score >= args.min_score {
            ...
            for (plat, tile, position, turn) in game_records {
                samples.push(Sample { plateau: plat, tile, position, turn, final_score, weight });
            }
        }
        ...
        if (game_idx + 1) % 1000 == 0 {
            ...
            println!("  Generated {}/{} | avg: {:.1} pts | kept: {} (>={}) | {:.0} g/s",
                     game_idx + 1, args.gen_games, avg, kept_games, args.min_score,
    ...
    println!("   {} samples from {}/{} games (avg: {:.1} pts) in {:.1}s",
             samples.len(), kept_games, args.gen_games, avg, gen_start.elapsed().as_secs_f32());
    samples
}
...
fn evaluate_gpu(
    net: &HypergraphTransformerPolicyNet,
    features_gpu: &Tensor, targets_gpu: &Tensor, masks_gpu: &Tensor,
    indices: &[usize], batch_size: usize, device: Device,
) -> (f64, f64) {
...
fn eval_games(net: &HypergraphTransformerPolicyNet, n_games: usize, rng: &mut StdRng, device: Device) -> (f64, Vec<i32>) {
