...
#[derive(Parser, Debug)]
#[command(
    name = "compare-mcts-qpolicy",
    about = "Compare Q-Net as Prior Policy strategies"
)]
struct Args {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // If Q-net ranking is useful, the best Q-net position should often be the best rollout position
    {
        ...
        for (i, &test_tile) in test_tiles.iter().enumerate() {
            ...
            let mut qnet_ranking: Vec<(usize, f64)> =
                empty.iter().map(|&p| (p, qvalues[p])).collect();
            ...
            log::info!(
                "   State {}: Q-net best={}, Rollout best={}, Q-net in top3={}",
                i,
                qnet_best,
                rollout_best,
                rollout_top3.contains(&qnet_best)
            );
        }

        log::info!(
            "   ACCURACY: Top-1={}/{} ({:.0}%), In-Top-3={}/{} ({:.0}%)",
            correct_top1,
            total_tests,
            correct_top1 as f64 / total_tests as f64 * 100.0,
            correct_top3,
            total_tests,
            correct_top3 as f64 / total_tests as f64 * 100.0
        );
    ...
    for strategy in &strategies {
        ...
        for game_idx in 0..args.games {
            ...
            let score = match *strategy {
                ...
                "progressive" => {
                    play_game_progressive(&tiles, args.simulations, &qnet, args.bias_coef)
                }
                "leaf" => play_game_leaf_init(&tiles, args.simulations, &qnet),
                "combined" => {
                    play_game_combined(&tiles, args.simulations, &qnet, args.top_k, args.bias_coef)
                }
    ...
    println!("{}", "=".repeat(60));
    println!(
        "Games: {}, Simulations: {}, Top-K: {}, Bias: {:.1}",
    ...
    for strategy in &[
        ...
        if let Some((mean, std, scores)) = results.get(*strategy) {
            ...
            println!(
                "{:12} : mean={:>6.2}, std={:>5.2}, delta={}, wins={}",
    ...
    if let (Some((_, _, _)), Some((prog_mean, _, _))) =
        (results.get("pure"), results.get("progressive"))
    {
...
fn find_best_position_prune(
    ...
    let top_positions: Vec<usize> = scored_positions
        .iter()
        .take(top_k)
        .map(|(pos, _)| *pos)
        .collect();
...
fn play_game_progressive(tiles: &[Tile], num_sims: usize, qnet: &QValueNet, bias_coef: f64) -> i32 {
    ...
    for tile in tiles {
        let best_pos =
            find_best_position_progressive(&plateau, &deck, *tile, num_sims, qnet, bias_coef);
        plateau.tiles[best_pos] = *tile;
        deck = replace_tile_in_deck(&deck, tile);
    }
...
fn play_game_combined(
    ...
    for tile in tiles {
        let best_pos =
            find_best_position_combined(&plateau, &deck, *tile, num_sims, qnet, top_k, bias_coef);
        plateau.tiles[best_pos] = *tile;
        deck = replace_tile_in_deck(&deck, tile);
    }
...
fn find_best_position_combined(
    ...
    let candidate_positions: Vec<usize> = scored_positions
        .iter()
        .take(k)
        .map(|(pos, _)| *pos)
        .collect();
...
fn find_best_position_softmax(
    ...
    let mut scored: Vec<(usize, f64)> = empty_positions
        .iter()
        .map(|&pos| (pos, qvalues[pos]))
        .collect();
    ...
    let num_candidates = (empty_positions.len() / 2)
        .max(3)
