...
use crate::test::{choisir_et_placer, create_plateau_empty, create_shuffle_deck, result, Tile};

mod test;
mod plateau;

/// Simule 100 parties à partir d'un état donné pour calculer le meilleur score
fn simulate_games(deck: &mut test::Deck, plateau: &mut plateau::Plateau) -> i32 {
...
fn find_best_position(deck: &mut test::Deck, plateau: &mut plateau::Plateau, tuile: Tile) -> (usize, i32) {
    let mut best_score = 0;
    ...
    for position in 0..plateau.tiles.len() {
        ...
        if plateau.tiles[position] == Tile(0, 0, 0) {
            ...
            let best_position_score = simulate_games(deck, &mut temp_plateau);
...
fn main() {
    ...
    let (best_position, best_score) = find_best_position(&mut deck, &mut plateau, tuile_choisie);
