...
fn test_enhance_game_state_populates_available_positions_immediately() {
    ...
    let enhanced_state: Value =
        serde_json::from_str(&enhanced_json).expect("Enhanced game state should be valid JSON");
    ...
    let player_plateaus = enhanced_state["player_plateaus"]
        .as_object()
    ...
    let available_positions = player_plateau["available_positions"]
        .as_array()
    ...
    for position in available_positions {
        let pos_num = position.as_i64().expect("Position should be a number");
        assert!(
            pos_num >= 0 && pos_num <= 18,
            "Position {} should be between 0 and 18",
            pos_num
        );
    }

    println!(
        "✅ Test réussi: enhance_game_state_with_images popule {} positions en {}ms",
...
fn test_empty_positions_are_detected_correctly() {
    ...
    let available_positions = enhanced_state["player_plateaus"][&player_id]["available_positions"]
        .as_array()
        .expect("Should have available_positions array");

    // Les positions 0, 5, 10 ne devraient PAS être disponibles
    let available_nums: Vec<i64> = available_positions
        .iter()
        .map(|v| v.as_i64().unwrap())
        .collect();

    assert!(
        !available_nums.contains(&0),
        "Position 0 should not be available (occupied)"
    );
    assert!(
        !available_nums.contains(&5),
        "Position 5 should not be available (occupied)"
    );
    assert!(
        !available_nums.contains(&10),
        "Position 10 should not be available (occupied)"
    );

    // Les autres positions devraient être disponibles
    assert!(
        available_nums.contains(&1),
        "Position 1 should be available (empty)"
    );
    assert!(
        available_nums.contains(&2),
        "Position 2 should be available (empty)"
    );
    ...
    assert_eq!(
        available_positions.len(),
        16,
        "Should have 16 available positions (19 - 3 occupied)"
    );

    println!(
        "✅ Test partiel réussi: {} positions disponibles sur 19",
        available_positions.len()
    );
}
